...
class Point(Dict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "batch" not in self.keys() and "offset" in self.keys():
           # print('offset', self["offset"])
            #print('offset_type', self["offset"].type()) 
            self["batch"] = offset2batch(self.offset)
            #print('Generated batch:', self["batch"].size())
            #print('batch_type', self["batch"].type())


        elif "offset" not in self.keys() and "batch" in self.keys():
            self["offset"] = batch2offset(self.batch)
            #print('Generated offset:', self["offset"].size())
    ...
    def serialization(self, order="z", depth=None, shuffle_orders=False):
        ...
        if len(self.grid_coord) != len(self.batch):
            raise ValueError("The size of 'grid_coord' must match the size of 'batch'.")
        ...
        # Ensure the tensors have contiguous memory layout
        self.grid_coord = self.grid_coord.contiguous()
        ...
        code = [
            encode(self.grid_coord.long(), self.batch.long(), depth, order=order_) for order_ in order
        ]
...
class SerializedAttention(PointModule):
    def __init__(
        ...
        if enable_flash:
            # print('Flash Attention is enabled.')
            assert (
                enable_rpe is False
            ...
            # print('Flash Attention is disabled, using standard attention instead.')
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        dec_channels=(192, 64, 128, 256),
        

        dec_num_head=(12, 4, 8, 16),                   
